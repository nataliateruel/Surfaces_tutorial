...
The values produced by Surfaces considering the atomic areas in contact and the pseudo-energy matrix are not bound to any particular unit. Considering that binding energies are often calculated as kcal/mol and taking 1101 experimental points used in the compared results showed in the article (from `Sirin et al., 2016 <https://pubmed.ncbi.nlm.nih.gov/26473627/>`_), we reached a scaling factor currently set as 2.43x10^-4 and used in **surface_cont.py**, **surface_cont_lig.py** and **surface_cont_res.py**.
